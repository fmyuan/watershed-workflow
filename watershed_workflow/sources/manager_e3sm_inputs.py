"""Manager for interacting with E3SM full input datasets."""
...
class FileManagerDaymet:
    ...
    E3SM inputdata is a very large and comprehensive datasets for 
    configuring and running individual or combinations ofcomponents
    of DOE Energy Efficient Earth System Models. 

    The purpose here is to obtain missing data and/or templates for 
    configuring and running E3SM Land Model (ELM) at user definable
    resolution or regions. In ELM-ATS coupled mode, it will generate
    ATS mesh based grid-system and its inputs to run ELM alone or coupled
    model.
    
    For such a purpose, E3SM components are in general:
     DATM%*_ELM%*_SICE_SOCN_MOSART_SGLC_SWAV_SIAC_SESP
    where, DATM%* - Data mode of Atmospheric component, with %* for varied configurations
           ELM%*  - full mode of Land component, with %* for varied configurations
           MOSART - River routine model, part of Land model, but may be off.
           SICE, SOCN, SGLC, SWAV, SIAC, SESP - components are in stab mode

         NOTE: in ELM configuration, there is a specific mode, Coupler Bypass mode, 
             at which ELM forcings are NOT via Coupler (CPL)-DATM, but directly read
             in ELM. So the E3SM components are:
              SATM_ELM%*_SICE_SOCN_MOSART_SGLC_SWAV_SIAC_SESP

    .. General data structure for DATM-ELM::
       e3sm/inputdata/atm/cam/chem/ ...
                    |        /datm7/CLM1PT_data/ ...
                    |              /atm_forcing.datm7.*/ ...
                    |              /CO2/ ...
                    |              /NASA_LIS/ ...
    ...
    .. [E3SM_INPUTDATA]https://web.lcrc.anl.gov/public/e3sm/inputdata/
    """
    
    # the following shall be under WW data directory
    DEFAULT_ROOT = 'e3sm/inputdata/'
    URL = "https://web.lcrc.anl.gov/public/e3sm/inputdata/"
