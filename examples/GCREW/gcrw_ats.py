"""
	Complete Workflow for generating ATS input for GCREW containing TEMPEST area
	...
	Given some basic inputs (in the next cell) including a NAME, 
	this workflow creates the following files, all of which will reside in output_data:
	
		- Mesh file: gcrew.exo, includes all labeled sets
		- Forcing: DayMet data -- daily raster of precip, RH, incoming radiation, etc.
		- GCREW_daymet_2010_2011.h5, the DayMet data on this watershed
		- GCREW_daymet_CyclicSteadystate.h5, a "typical year" of DayMet, smoothed for spinup purposes, then looped certain number of years
		- Forcing: LAI data -- every 4 days, time series by land cover type of LAI.
		- GCREW_LAI_MODIS_transient.h5, the LAI, interpolated and smoothed from the raw MODIS data
		- GCREW_LAI_MODIS_CyclicSteadystate.h5, a "typical year" of LAI, smoothed for spinup purposes then looped 10 years
		
	ATS Input files for three runs, intended to be run sequentially:
		- GCREW_steadystate.xml the steady-state solution based on uniform application of mean rainfall rate
		- GCREW_cyclic_steadystate.xml the cyclic steady state based on typical years
		- GCREW_transient.xml the forward model, run from 2010 -- 2011
